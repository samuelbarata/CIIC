...
# Rules
RC_1 = "IF (CpuMem IS high) THEN CLP IS increase_much"
RC_2 = "IF (CpuMem IS high) AND (OutCongestion IS low) THEN CLP IS decrease_much"
RC_3 = "IF (CpuMem IS high) THEN CLP IS decrease"
...
if __name__ == '__main__':
    ...
    for index, row in df.iterrows():
        ...
        print(f"Predicted CLP: {int(round(clp, 2)*100)}%; Expected CLP: {int(clp_variation*100)}%")
